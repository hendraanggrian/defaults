...
interface Prefs {
    ...
    companion object {
        ...
        private fun findBindingConstructor(cls: Class<*>): Constructor<PrefsSaver>? {
            ...
            if (binding != null) {
                PrefsInternal.debug("HIT: Cache found in binding weak map.")
                return binding
            }
            if (cls.name.startsWith("android.") || cls.name.startsWith("java.")) {
                PrefsInternal.debug("MISS: Reached framework class. Abandoning search.")
                return null
            }
            try {
                binding = cls.classLoader!!
                    .loadClass(cls.name + BindPref.SUFFIX)
                    .getConstructor(cls, Prefs::class.java) as Constructor<PrefsSaver>
                PrefsInternal.debug("HIT: Loaded binding class, caching in weak map.")
            } catch (e: ClassNotFoundException) {
                val superclass = cls.superclass
                PrefsInternal.debug("Not found. Trying superclass ${superclass!!.name}")
